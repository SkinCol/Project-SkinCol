<%-- 
    Document   : editarEnvio
    Created on : 1/08/2019, 01:13:23 PM
    Author     : angel
--%>
                  ...
                  <div class="card-body px-lg-5 pt-5">
                    ...
                    <form class="mb-3 pb-1" name="formulario" id="formulario" method="POST">
                      ...
                      <label for="select">Usuario</label>
                        <select class="browser-default custom-select mb-4" name="IdUsuario" id="IdUsuario">
                            <c:forEach var="dato" items="${listaU}">
                                <c:if test="${dato.IdEnvio != lista[0].IdUsuario}">
                                    <option value="${dato.IdUsuario}">
                                        ${dato.Nombres}
                                    </option>
                                </c:if>
                                    <c:if test="${dato.Envio == lista[0].IdUsuario}">
                      ...
                      <label for="select">Factura</label>
                        <select class="browser-default custom-select mb-4" name="IdFactura" id="IdFactura">
                            <c:forEach var="dato" items="${listaF}">
                                <c:if test="${dato.IdEnvio != lista[0].IdFactura}">
                                    <option value="${dato.IdFactura}">
                                        ${dato.IdFactura}
                                    </option>
                                </c:if>
                                    <c:if test="${dato.Envio == lista[0].IdFactura}">
                                        <option value="${dato.IdFactura}" selected="true">
                                        ${dato.IdFactura}
                                        </option>                                                                          
                                </c:if>
                            </c:forEach>
                      ...
                      <div class="form-group col-sm-12 col-md-6 col-xl-12">
                        <label>Fecha</label>
                        <input type="date" name="Fecha" id="Nombre" class="form-control" value="${lista[0].Fecha}">
                      </div>

                      <hr>
                      <button class="btn btn-outline-success btn-block" type="submit" value="Guardar"><i class="fa fa-save"></i>
                        Guardar</button>
                      <a class="btn btn-outline-danger btn-block" href="indexEnvio.htm"><i
