<%-- 
    ...
    Created on : 31/07/2019, 08:38:59 PM
    Author     : angel
--%>
                ...
                <div class="card">
                  ...
                  <div class="card-body px-lg-5 pt-5">
                    ...
                    <form class="mb-3 pb-1" name="formulario" id="formulario" method="POST">
                      ...
                      <label for="select">Solicitud</label>
                        <select class="browser-default custom-select mb-4" name="IdSolicitud" id="IdSolicitud">
                            <c:forEach var="dato" items="${listaS}">
                                <c:if test="${dato.IdFactura != lista[0].IdSolicitud}">
                                    <option value="${dato.IdSolicitud}">
                                        ${dato.CostoSkin}
                                    </option>
                                </c:if>
                                    <c:if test="${dato.Factura == lista[0].IdSolicitud}">
                                        <option value="${dato.IdSolicitud}" selected="true">
                                        ${dato.CostoSkin}
                                        </option>                                                                          
                                </c:if>
                            </c:forEach>
                      ...
                      <label for="select">Usuario</label>
                        <select class="browser-default custom-select mb-4" name="IdUser" id="IdUser">
                            <c:forEach var="dato" items="${listaU}">
                                <c:if test="${dato.IdFactura != lista[0].IdUser}">
                                    <option value="${dato.IdUser}">
                                        ${dato.Nombres}
                                    </option>
                                </c:if>
                                    <c:if test="${dato.Factura == lista[0].IdUser}">
